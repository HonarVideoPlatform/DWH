...
CREATE PROCEDURE `daily_procedure_dwh_aggr_events_widget`(date_val DATE,aggr_name VARCHAR(100))
BEGIN
	DECLARE aggr_table VARCHAR(100);
	DECLARE aggr_id_field VARCHAR(100);
...
CREATE PROCEDURE `daily_procedure_dwh_aggr_partner_daily_usage`(date_val DATE)
BEGIN
...
CREATE PROCEDURE `daily_procedure_dwh_aggr_partner_daily_usage_loop`(from_date DATE,to_date DATE)
BEGIN
