...
CREATE PROCEDURE `generate_QoS_report`(p_date_val DATE)
BEGIN
	...
	FROM
	(
		SELECT 	IF(entry_media_type_id NOT IN (1, 5, 2, 6), -1, entry_media_type_id) entry_media_type_id,
			SUM(IF (updated_at BETWEEN DATE(yesterday_date_id) AND DATE(yesterday_date_id) + INTERVAL 1 DAY, 1, 0)) yesterday,
			SUM(IF (updated_at BETWEEN DATE(the_day_before_yesteray_date_id) AND DATE(the_day_before_yesteray_date_id) + INTERVAL 1 DAY, 1, 0)) the_day_before,
			SUM(IF (updated_at BETWEEN DATE(5_days_ago_date_id) AND DATE(yesterday_date_id) + INTERVAL 1 DAY, 1, 0))/5 last_5_days,
			COUNT(*)/30 last_30_days			
			FROM kalturadw.dwh_dim_entries 
			WHERE updated_at BETWEEN DATE(30_days_ago_date_id) AND DATE(yesterday_date_id) + INTERVAL 1 DAY
			GROUP BY IF(entry_media_type_id NOT IN (1, 5, 2, 6), -1, entry_media_type_id)
	) e RIGHT OUTER JOIN 
	(
		...
		FROM kalturadw.dwh_dim_entry_media_type
		WHERE entry_media_type_id IN (1, 5, 2, 6, -1)
	) t
