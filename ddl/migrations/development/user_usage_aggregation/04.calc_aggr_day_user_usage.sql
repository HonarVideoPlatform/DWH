...
BEGIN
    ...
    INSERT INTO     temp_aggr_storage (partner_id, kuser_id, count_storage_kb)
       SELECT         e.partner_id, e.kuser_id, SUM(f.entry_additional_size_kb) count_storage_kb
    ...
    INSERT INTO  temp_aggr_storage (partner_id, kuser_id, count_storage_kb)
    SELECT       o.partner_id, o.prev_kuser_id, SUM(f.entry_additional_size_kb)*-1  count_storage_kb
    ...
    INSERT INTO  temp_aggr_storage (partner_id, kuser_id, count_storage_kb)
    SELECT       o.partner_id, o.kuser_id, SUM(f.entry_additional_size_kb)  count_storage_kb
    FROM         dwh_fact_entries_sizes f, entries_prev_owner o
    WHERE        f.entry_id = o.entry_id
    AND          f.entry_size_date_id < p_date_id
    GROUP BY     o.kuser_id;
